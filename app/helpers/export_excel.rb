    ...
    if(t_ob=="other_image_url1")
      ...
      code.each do |b|
        ...
        otherphotos.each do |d|
          ...
          if(b =~ (/^sku/))
            name =d.name[0,5].downcase
          end
          if b==name
            if m<otherimg_num+t_num
              if j==1 #第一行
                #sheet1[1+c_cloum,m] = geturl(d.picture.url) #
                sheet1[1+c_cloum,m] = url_for(d.picture)
              end
              #其他行
              csize.each_with_index do |c,index|
                sheet1[j+index+1+c_cloum,m] = url_for(d.picture)
              end
            end
            m+=1
          end
  ...
  if(t_ob=="leg_diameter"&& !leg_diameter.empty?)
    ...
    if(t_ob=="other_image_url8")
      if sizeob
        m = t_num
        sizej=1
        url=geturl(url_for(sizeob.picture))
    ...
    File.open(file_path, 'r') do |f|
      ...
      :filename => filename, :type => "application/xls",disposition: 'attachment'

    end
    #return file_path
    # send_file(file_path+filename,filename: filename,
